...
* `与(&)` 与运算，当相同位都为1时，结果为1。
 + 判断奇偶性。`n&1`，结果为1是奇数，结果为0是偶数，这种比n%2这种要快。
...
　　在前面提到过，安卓的SDK中经常可以见位掩码，据说是因为枚举占用的空间比较多，但在实际开发中，并不需要像基本库那样考虑存储效率（如果这点空间都需要考虑，那就应该考虑整体的设计了）。其实在Java的基本库中，有一个将枚举和位掩码结合的类，那就是`java.util.EnumSet`。
